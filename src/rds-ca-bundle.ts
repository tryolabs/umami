/**
 * Amazon RDS certificate authority bundle (all regions), inlined as a string.
 *
 * Fork-specific file, not part of upstream umami. Used by `src/instrumentation.ts`;
 * see that file for why this exists.
 *
 * Source: https://truststore.pki.rds.amazonaws.com/global/global-bundle.pem
 * (same contents as `certs/global-bundle.pem`). It is inlined as code rather than read
 * from disk so that it is compiled into the Netlify function bundle and needs no file
 * path at runtime.
 *
 * To update after AWS rotates its CAs, download a fresh bundle to
 * `certs/global-bundle.pem` and regenerate this file from it.
 */
